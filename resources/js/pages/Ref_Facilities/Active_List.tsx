...
import { Pencil, Trash2, TableOfContents, Eye } from "lucide-react";
import {
...
type ListProps = {
  refreshKey: any;
  id: string | null;
  setSelectedFacilities: (ids: string[]) => void; // <- add this
};

const Active_List = ({ refreshKey, id,setSelectedFacilities }: ListProps) => {
  ...
  useEffect(() => {
    ...
    setSelectedRows([]);
    setSelectedFacilities([]); // <-- add this line to stay in sync
  }, [refreshKey, page, searchTerm, perPage]);
  
  useEffect(() => {
    console.log("Selected Rows:", selectedRows);
    setSelectedFacilities(selectedRows);
  }, [selectedRows]);
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      setSelectedRows(data.map(row => row.hfhudcode));  // Select all rows
    } else {
    ...
    <div className="p-3 mr-2 ml-2 mt-1">
      ...
      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-lg">Active facilities</h2>
        </div>
        <Input
          type="text"
          placeholder="Search..."
      ...
      {selectedRows.length > 0 && (
        <div className="mb-3 text-xs text-muted-600 flex justify-between items-center">
            ...
            <Table className="text-sm">
              ...
              <TableBody>
                {sortedData.length > 0 ? sortedData.map((row) => (
                  <TableRow key={row.hfhudcode}>
                    {visibleColumns.checkbox && (
                      <TableCell className="p-1">
                        <input
                          type="checkbox"
                          checked={selectedRows.includes(row.hfhudcode)}
                          onChange={(e) => handleCheckboxChange(row.hfhudcode, e)}
                        />
                      </TableCell>
                    )}
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-gray-500 italic py-4">
                      No results found.
                    </TableCell>
                  </TableRow>
                )}

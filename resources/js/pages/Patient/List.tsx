...
const Lists = ({ canEdit,canDelete,refreshKey, onEdit }:Props) => {
  ...
  const fetchData = async (pageNumber = 1, search = "") => {
    setLoading(true);
    try {
      const response = await axios.get(`/patient-list?page=${pageNumber}&search=${search}`);
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead >
              <tr>
                ...
                <th className="px-2 py-1 text-left font-medium">Date of birth</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {data.length > 0 ? (
                data.map((row) => (
                  <tr key={row.LogID}>
                    <td className="px-2 py-1">{row.patient_name}</td>
                    <td className="px-2 py-1">{row.sex}</td>
                    <td className="px-2 py-1">{row.birthdate
                      }</td>
